...
describe('PID Controller', function() {
	describe('proportional calculation', function() {
		it('returns positive value when input is below setpoint', function() {
			var pid = new Pid(10, 'test', 2, 0, 0, mockTemperatureReader);
	...
	describe('integral calculation', function() {
		it('returns positive value when input is below setpoint', function() {
			var pid = new Pid(10, 'test', 0, 2, 0, mockTemperatureReader);
			pid._integral(2);
	...
	describe('differential calculation', function() {
		it('returns positive value when input is below setpoint', function() {
			var pid = new Pid(10, 'test', 0, 0, 2, mockTemperatureReader);
			pid._differential(7);
	...
	it('stabilizes a system with some lag', function() {
		...
		var pid = new Pid(setPoint, 'test', 0.2, 0.2, 1, mockTemperatureReader);
		var lastPidOutput;
